    ...
    const options = {
        ...
        editor: {
          ...
          path: {   // Each supports path variables (e.g. {ORIGINAL_FILE_EXT}). See your
           folderPath: `/uploads/suachave/imóveis/${user.id}/Documents | ${new Date().getDate()}-${new Date().getMonth()+1}-${new Date().getFullYear()} | ${new Date().getHours()}:${new Date().getMinutes()}`     // API key's config in the Upload Dashboard for all path variables.
          },
     }
  ...
  <UploadButton uploader={uploader}         // Required.
    ...
    {({onClick}) =>
      <button className="btnDocuments" onClick={onClick}>
        <IoArrowUpCircle/> Adicionar comprovantes 
      </button>
    }
  </UploadButton>
    )
